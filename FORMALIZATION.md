...
| `thm:universal-lift` | Universal block-feature lift | open | Rosenberg and finite block identities have computational checks only. |
| `thm:algebraic-certificate` | Marginal-ideal certificate | partial | `MarginalTradeCertificate` and `MarginalTradeCertificate.obstructs_localization` in `KLocality.MarginalTradeCertificate` give a uniform finite checker for arbitrary `k`, homogeneous degree, finite observed type, and finite latent type. Soundness is boundary-safe: equal joint feature profiles imply equal probability products for every face--Gibbs law, including zeros on proper faces. `twoCubicDistribution_localizationComplexity_eq_two` checks a full-support rational five-bit law with exact `LC_2(D)=2`; `cubicHardDistribution_localizationComplexity_gt_one` proves `LC_3(D)>1` for an explicit ten-bit law. More substantially, `superlinearCubicDistribution_localizationComplexity_gt` in `KLocality.UniformExplicitCubicLowerBound` proves for every `m` that an explicit full-support rational law on `4m+24` bits satisfies `LC_3(D_m)>2^m`. The literal corollary `superlinearCubicDistribution_localizationComplexity_gt_sq` proves a quadratic-in-visible-dimension lower bound for `m>=13`. The proof uses parameterized finite profile collisions against every hidden count through `2^m`; collided subsets are chosen from the finite proof rather than enumerated. The manuscript's all-`k` elimination theorem, sharp `2^(n/3)` cubic scale, and general degree bound remain absent. |
| `prop:certificate-degree` | Explicit certificate degree | open | Requires the substitution linear map and exact dimension/counting inequalities. |
| `cor:effective-tables` | Effective rational hard tables and Gibbs rays | partial | `KLocality.UniformExplicitCubicLowerBound` checks a superlinear parameterized base-coded construction: for every `m`, a closed-form positive rational table on `4m+24` bits has `LC_3(D_m)>2^m`. The proof derives collisions by finite pigeonhole choice and gives degree `2(2^(4m+24)-1)` certificates rather than computing their terms. The largest probability numerators have doubly exponential binary length (and numerical value one exponential level larger), so this is mathematical explicitness rather than a polynomial-bit table generator. It does not yet recover the manuscript's all-`k` eliminant, sharp `2^(n/k)` scale, practical bit complexity, or Gibbs-ray statement. |
| `thm:generic-positive` | Generic full-support lower bound | open | Requires the marginal-ideal dimension theorem and a measure-zero result for proper real algebraic sets. |
| `cor:worst-case` | Worst-case localization complexity | open | Depends on the universal lift and generic lower bound. |
