...
\textit{$T \subseteq S$:} We argue that the maximum entropy optimizer assigns positive probability to every point in $T$. Consider the maximum entropy problem restricted to distributions supported on $T$. The feasible set is nonempty (the constraints are consistent by assumption) and the entropy function is strictly concave, so the optimizer exists and is unique. By the theory of exponential families (see \cite[Theorem~3.4]{wj08}), the optimizer takes the Gibbs form
\[ \rho(x) = \exp\!\Big(\lambda_0 + \sum_i \lambda_{i, x_{S_i}}\Big) \]
for all $x \in T$, where $\lambda_0$ is a normalizing constant and the $\lambda_{i, x_{S_i}}$ are Lagrange multipliers for the marginal constraints. The critical observation is that the exponential function is strictly positive: for any real values of the multipliers, $\rho(x) > 0$ for all $x \in T$. This positivity is guaranteed because no marginal constraint forces $\rho(x) = 0$ for $x \in T$---indeed, for every $x \in T$ and every $i$, we have $\phi_i(x_{S_i}) > 0$ by definition of $T$, so the constraint $\phi_i$ is compatible with $x$ receiving positive mass. (This is the positivity condition that underlies the Hammersley--Clifford theorem \cite{hc71}: the Gibbs factorization is valid precisely on the positive part of the distribution.) Therefore $T \subseteq S$.
\end{proof}
...
\[ \mathsf{generates}_k(C, D) \equiv \forall x \in \{0, 1\}^n, \rho(x) = Pr_{x' \sim U_{i(C)}}(C(x') = x). \]
For any given $k$, we define the generator complexity of the distribution $D$,
\[ G_k(D) = \min_{C : \mathsf{generates}_k(C, D)} i(C) + s(C) \]
\end{definition}
...
by the definition of $\mathsf{generates}_k$. Thus $D'$ is a $k$-localization of $D$ with $m + s$ hidden variables.
\end{proof}
...
\p The above is a nice link between the work in \cite{vi10} and our work here, showing that our model is at least as expressive as the model introduced there.

\p It is worth noting that Viola \cite{vi10} introduces several complexity measures beyond generator complexity, including \emph{distinguishing complexity} (how large a circuit is needed to distinguish $D$ from uniform) and \emph{sampling complexity} (the minimum circuit size needed to sample from $D$ given access to random bits). Viola proves relationships among these measures using communication complexity techniques---in particular, showing that distributions requiring large communication to generate also require large circuits to sample. An interesting question is whether these communication complexity techniques could yield lower bounds on $\LC_k$. The marginal constraints in a $k$-localization define a structured form of interaction between the observed and latent variables, and it is plausible that communication lower bounds (e.g., via information complexity) could translate into constraints on the number of latent variables needed. We leave this as a direction for future work.

\p The construction shown above is generic, in the sense that this idea of using the marginals as local verification of circuit operation can be used to show that our model is upper bounded by a number of notions of circuit complexity, a few examples of which will now be proven.
...
and we define the proposition $\mathsf{witcount}_k(C, D)$ to state that $C$ is a witness counting model for $D$. We define the witness counting complexity of $D$ as
\[ W_k(D) = \min_{C : \mathsf{witcount}_k(C, D)} m(C) + s(C). \]
\end{definition}
...
\p The analogy with proof complexity is worth developing more carefully. In our constructions, the latent variables play the role of a ``proof'' or ``witness'' that $x \in S$, while the $k$-marginal constraints serve as ``local checks'' that verify this proof by inspecting at most $k$ variables at a time. We can make this correspondence precise:

\begin{itemize}
\item \textbf{Proof string.} Given a $k$-localization of $D$ with $l$ latent variables, an assignment $h \in \{0,1\}^l$ to the latent variables serves as a ``proof'' that the observed string $x$ belongs to the support $S$.
\item \textbf{Local checks.} Each marginal constraint $\phi_i$ on variables $S_i$ with $|S_i| \leq k$ acts as a local check: it reads at most $k$ bits of the combined string $(x, h)$ and verifies that the observed pattern is consistent (i.e., lies in $\mathrm{supp}(\phi_i)$).
\item \textbf{Global consistency.} In the PCP setting \cite{as98, almo98}, probabilistic verification ensures that a globally invalid proof is rejected with high probability. In our setting, entropy maximization plays an analogous role: the support intersection lemma (lemma~\ref{lem:support-intersection}) ensures that the \emph{only} configurations receiving positive probability are those passing \emph{all} local checks simultaneously.
\end{itemize}

\p This suggests viewing $\LC_k(D)$ through the lens of proof systems: the proof length is $l = \LC_k(D)$ (the number of latent variables), and the query complexity is $k$ (the locality of each check). Unlike PCPs, where the verifier is randomized and may err, our ``verifier'' is deterministic---it accepts exactly when all marginal constraints are satisfied---but it achieves global soundness through the maximum entropy principle rather than through random sampling of checks. It is an interesting open question whether techniques from algebraic proof systems or the sum-check protocol \cite{lfkn92} could be adapted to yield tighter bounds on $\LC_k$, particularly for structured distributions whose supports have algebraic descriptions.
...
\begin{remark}[On the gap between upper and lower bounds]\label{rem:gap}
For flat distributions, the gap between our best upper bound $\LC_k(D) \leq C_{k-1}(S)$ and our lower bound $\LC_k(D) = \Omega_k(\mathrm{NSize}(S)^{1/k}) - n$ deserves closer examination. For a set $S$ with $\mathrm{NSize}(S) = t$ and $C_{k-1}(S) = s$, our bounds give roughly $t^{1/k} \lesssim \LC_k(D) \lesssim s$, which can be an enormous gap since $s$ may be exponentially larger than $t^{1/k}$.

The $1/k$ exponent in the lower bound arises from our use of the crude bound $m \leq \binom{n+l}{k}$ on the number of marginal constraints. An optimal $k$-localization need not use all $\binom{n+l}{k}$ possible constraints---it may use far fewer---and so a structural argument that accounts for the actual constraint pattern could improve the exponent. In particular, if one could show that the constraint hypergraph of any $k$-localization achieving the minimum has some structural property (e.g., bounded overlap or expansion), this might yield a polynomial improvement in the lower bound.

A natural question is whether $\LC_k$ is genuinely a new measure, or whether it is equivalent to circuit complexity up to polynomial factors. We conjecture that $\LC_k$ and $C_{k-1}$ are \emph{not} polynomially related in general: a $k$-localization can exploit the structure of marginal constraints more flexibly than a circuit, since the constraints need not form a DAG and the latent variables need not correspond to gate computations. In particular, a $k$-localization allows ``sharing'' of information between constraints in ways that a circuit's acyclic structure does not. Exhibiting an explicit family of flat distributions where $\LC_k(D) \ll C_{k-1}(S)$ would confirm this separation and demonstrate that the maximum entropy framework captures computational structure beyond what circuits alone can express.
\end{remark}
...
The case $k = 2$ is genuinely different and merits special attention. The usefulness step above requires that fan-in-$(k-1)$ circuits can simulate general computation, which fails for $k = 2$: fan-in-$1$ circuits compute only functions of a single variable. The natural proofs barrier therefore does not apply, leaving the computational complexity of $\LC_2$ wide open.

The $2$-local distributions are precisely the pairwise Markov random fields, which include the Ising model as a special case---one of the most studied objects in statistical physics, combinatorics, and theoretical computer science. The computational complexity of pairwise models is rich: exact inference is $\#$P-hard in general, but the celebrated work of Jerrum and Sinclair \cite{js93} shows that approximate sampling from ferromagnetic Ising models is possible in polynomial time via Markov chain Monte Carlo. More broadly, dichotomy theorems for counting CSPs characterize which constraint languages admit efficient counting and which are $\#$P-hard.

For $\LC_2$ specifically, the question is: given a distribution $D$ (as a truth table) and a threshold $l$, can we decide whether $\LC_2(D) \leq l$ in polynomial time? Note that the upper bound $\LC_2(D) \leq C_1(S)$ is uninformative, since fan-in-$1$ circuits can only compute functions of a single variable and thus $C_1(S)$ is enormous for most sets $S$. This means that the relationship between $\LC_2$ and circuit complexity is genuinely unclear: unlike for $k \geq 3$, we have no useful circuit-based upper bound, so $\LC_2$ may capture a fundamentally different aspect of distributional structure. Resolving whether $\LC_2$ is computable in polynomial time, NP-hard, or of intermediate complexity would shed light on the power of pairwise constraints as a model of computation.
\end{remark}
...
Can we exhibit explicit families of distributions $\{D_n\}$ for which $\LC_k(D_n)$ is super-polynomial in $n$ for some fixed $k$? By our lower bound, this would require distributions whose supports have super-polynomial nondeterministic circuit complexity---itself a major open problem in circuit complexity, as no explicit super-polynomial lower bounds on $\mathrm{NSize}$ are known for functions in NP. This problem is therefore at least as hard as proving nondeterministic circuit lower bounds. However, the maximum entropy perspective may suggest new attack strategies: rather than directly constructing hard functions, one might identify distributions whose entropy-maximizing structure inherently resists efficient localization. Conversely, proving strong upper bounds on $\LC_k$ for all distributions would yield new circuit upper bounds.
\end{open}
...
Given a $k$-localization with marginal constraints $\{\phi_i\}_{i \in [m]}$, the \emph{constraint hypergraph} has vertex set $\{X_1, \ldots, X_n, H_1, \ldots, H_l\}$ and a hyperedge $S_i$ for each constraint $\phi_i$. Since each $|S_i| \leq k$, the primal graph (connecting any two variables appearing in a common constraint) has cliques of size at most $k$. The \emph{treewidth} of this primal graph \cite{rs86, bo93} is a natural parameter: when treewidth is bounded, inference in graphical models (including maximum entropy computation) can be performed efficiently via junction tree algorithms \cite{la96}. It is therefore plausible that for distributions $D$ whose optimal $k$-localization has a constraint graph of bounded treewidth, $\LC_k(D)$ can be computed in polynomial time---complementing our hardness result for the general case. Conversely, treewidth lower bounds for the constraint graph might yield improved lower bounds on $\LC_k$, since treewidth controls the complexity of inference and verification in graphical models.
\end{open}
