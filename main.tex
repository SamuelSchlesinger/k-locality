...
\begin{abstract}
We define the $k$-localization complexity $\LC_k(D)$ of a distribution $D$ as the minimum number of latent variables needed to realize $D$ as the observed marginal of a $k$-local maximum-entropy model. We isolate one technical mechanism---\emph{local verification}---that converts bounded fan-in deterministic circuit computations into $k$-local models. This yields:
\[
\LC_k(D) \leq G_{k-1}(D),\qquad
\LC_k(D) \leq C_{k-1}(S) \text{ for flat } D.
\]
The paper is intentionally restricted to foundational definitions, the local-verification theorem, and these deterministic circuit upper bounds.
\end{abstract}
...
The purpose of this paper is narrow: define the model precisely and prove its direct connections to bounded fan-in circuit complexity. We do not pursue algorithmic hardness of computing $\LC_k$ from an explicit input representation here.
...
\item Using this theorem, we prove upper bounds from two deterministic circuit measures: generator complexity and flat-support circuit complexity.
\end{itemize}
...
We now instantiate Theorem~\ref{thm:local-verification} for two deterministic circuit models.

\begin{definition}
Let $C_r(S)$ be the minimum number of gates (including the output gate) in a fan-in-$r$ deterministic circuit that recognizes $S\subseteq\{0,1\}^n$.
\end{definition}
...
\section{Summary}
The paper isolates one reusable proof pattern: local circuit consistency constraints can force global trace validity, and once this local completeness condition is met, uniform trace distributions are maximum-entropy $k$-local models. This gives direct upper bounds relating localization complexity to bounded fan-in deterministic circuit measures.
