...
| `prop:exchangeable-tv-collapse` | Approximation of the analytic exchangeable family | open | Manuscript proof gives an explicit total-variation bound and eventual zero approximate complexity. The concentration and normalized-density argument are not formalized. |
| `lem:parity-threshold-discrepancy` | Parity threshold discrepancy | open | The manuscript includes the attributed voting-polynomial dimension argument and its central-binomial consequence. No Lean threshold-discrepancy API or counterpart is present. |
| `lem:threshold-discrepancy-transfer` | Threshold discrepancy transfer | open | Manuscript proof handles each boundary Gibbs slice by a degree-preserving threshold and integrates clipped densities. The quantitative finite-mixture inequality and its model bridge are not formalized. |
| `thm:robust-noisy-parity` | Robust noisy-parity scale | open | Manuscript proof gives the fixed-error lower bound, a matching central-window quadratic lift, the same asymptotic for strictly positive joint lifts, and the exact uniform-approximation threshold. Finite rational validation checks declared instances only; all asymptotic and model-level clauses remain unformalized. |
